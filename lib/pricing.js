...
export function computeDiscount(voucher, subtotal) {
  ...
  let raw = voucher.type === "percent" ? Math.round((subtotal * voucher.value) / 100) : voucher.value;
  // Batasi maksimal potongan kalau di-set
  if (voucher.max_discount && Number(voucher.max_discount) > 0) raw = Math.min(raw, Number(voucher.max_discount));
